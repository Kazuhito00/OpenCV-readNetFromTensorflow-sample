...
class SSDAnchorGenerator:
    ...
    def get(self, layer_id):
        if layer_id == 0 and self.reduce_boxes_in_lowest_layer:
            widths = [
                0.1, self.min_scale * sqrt(2.0), self.min_scale * sqrt(0.5)
            ]
            heights = [
                0.1, self.min_scale / sqrt(2.0), self.min_scale / sqrt(0.5)
            ]
            ...
            widths = [
                self.scales[layer_id] * sqrt(ar) for ar in self.aspect_ratios
            ]
            heights = [
                self.scales[layer_id] / sqrt(ar) for ar in self.aspect_ratios
            ]
            ...
            heights += [
                sqrt(self.scales[layer_id] * self.scales[layer_id + 1])
            ]
...
class MultiscaleAnchorGenerator:
    def __init__(self, min_level, aspect_ratios, scales_per_octave,
                 anchor_scale):
    ...
    def get(self, layer_id):
        ...
        for a in self.aspect_ratios:
            for s in self.scales:
                base_anchor_size = 2**(
                    self.min_level + layer_id) * self.anchor_scale
                ar = sqrt(a)
...
def createSSDGraph(modelPath, configPath, outputPath):
    ...
    keepOps = [
        'Conv2D', 'BiasAdd', 'Add', 'AddV2', 'Relu', 'Relu6', 'Placeholder',
        'FusedBatchNorm', 'DepthwiseConv2dNative', 'ConcatV2', 'Mul',
        'MaxPool', 'AvgPool', 'Identity', 'Sub', 'ResizeNearestNeighbor',
        'Pad', 'FusedBatchNormV3', 'Mean'
    ]

    # Node with which prefixes should be removed
    prefixesToRemove = ('MultipleGridAnchorGenerator/', 'Concatenate/',
                        'Postprocessor/', 'Preprocessor/map')
    ...
    box_predictor = 'convolutional' if 'convolutional_box_predictor' in config[
        'box_predictor'][0] else 'weight_shared_convolutional'
    ...
    if 'ssd_anchor_generator' in anchor_generator:
        ...
        aspect_ratios = [
            float(ar) for ar in ssd_anchor_generator['aspect_ratios']
        ]
        reduce_boxes_in_lowest_layer = True
        if 'reduce_boxes_in_lowest_layer' in ssd_anchor_generator:
            reduce_boxes_in_lowest_layer = ssd_anchor_generator[
                'reduce_boxes_in_lowest_layer'][0] == 'true'
        priors_generator = SSDAnchorGenerator(
            min_scale, max_scale, num_layers, aspect_ratios,
            reduce_boxes_in_lowest_layer, image_width, image_height)

        print('Scale: [%f-%f]' % (min_scale, max_scale))
        print('Aspect ratios: %s' % str(aspect_ratios))
        print('Reduce boxes in the lowest layer: %s' %
              str(reduce_boxes_in_lowest_layer))
    elif 'multiscale_anchor_generator' in anchor_generator:
        multiscale_anchor_generator = anchor_generator[
            'multiscale_anchor_generator'][0]
        ...
        scales_per_octave = int(
            multiscale_anchor_generator['scales_per_octave'][0])
    ...
    def fuse_nodes(nodesToKeep):
        ...
        subgraphBatchNorm = [
            ...
            [
                'Mul', 'input',
                [
                    'Mul', ['Rsqrt', ['Add', 'moving_variance', 'add_y']],
                    'gamma'
                ]
        ...
        subgraphBatchNormV2 = [
            ...
            [
                'Mul', 'input',
                [
                    'Mul', ['Rsqrt', ['AddV2', 'moving_variance', 'add_y']],
                    'gamma'
                ]
            ], ['Sub', 'beta', ['Mul', 'moving_mean', 'Mul_0']]
        ]
        ...
        subgraphResizeNN = [
            ...
            [
                ...
                [
                    'Reshape', 'input',
                    [
                        'Pack', 'shape_1', 'shape_2', 'shape_3', 'shape_4',
                        'shape_5'
                    ]
                ], 'ones'
            ],
            [
                'Pack',
                [
                    'StridedSlice', ['Shape', 'input'], 'stack', 'stack_1',
                    'stack_2'
        ...
        for node in graph_def.node:
            ...
            if checkSubgraph(node, subgraphResizeNN, inputs, fusedNodes):
                ...
                out_width = int(
                    out_width_node.attr['value']['tensor'][0]['int_val'][0])
    ...
    for label in [
            'ClassPredictor', 'BoxEncodingPredictor'
            if box_predictor is 'convolutional' else 'BoxPredictor'
    ]:
        concatInputs = []
        for i in range(num_layers):
            # Flatten predictions
    ...
    for i in range(num_layers):
        ...
        if box_predictor is 'convolutional':
            ...
            if i == 0:
                ...
                priorBox.input.append(
                    'WeightSharedConvolutionalBoxPredictor_%d/BoxPredictor/BiasAdd'
                    % i)
    ...
    addReshape('ClassPredictor/concat', 'ClassPredictor/concat3d',
               [0, -1, num_classes + 1], graph_def)
    ...
    if 'iou_threshold' in batchNMS:
        detectionOut.addAttr('nms_threshold',
                             float(batchNMS['iou_threshold'][0]))
    else:
        detectionOut.addAttr('nms_threshold', 0.6)

    if 'score_threshold' in batchNMS:
        detectionOut.addAttr('confidence_threshold',
                             float(batchNMS['score_threshold'][0]))
    else:
        detectionOut.addAttr('confidence_threshold', 0.01)

    if 'max_detections_per_class' in batchNMS:
        detectionOut.addAttr('top_k',
                             int(batchNMS['max_detections_per_class'][0]))
    else:
        detectionOut.addAttr('top_k', 100)

    if 'max_total_detections' in batchNMS:
        detectionOut.addAttr('keep_top_k',
                             int(batchNMS['max_total_detections'][0]))
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--output', required=True, help='Path to output text graph.')
    parser.add_argument(
        '--config',
        required=True,
        help='Path to a *.config file is used for training.')
